...
class Selfie_model extends CI_Model {
    ...
    public function upload_selfie($user_id, $upload_data, $post) 
    {
        $data = array(
           'social_user_id' => $user_id,
           'user_journal_post_type' => 'selfie',
    ...
    public function get_selfie_by_user_id($user_id) 
    {
        $this->db->select('*');
        ...
        $this->db->where('user_journal_posts.user_journal_post_type', 'selfie');
